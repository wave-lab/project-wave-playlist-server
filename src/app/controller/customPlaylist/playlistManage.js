...
router.post('/', async (req, res) => {
    ...
    if(playlistSelect.length != 0) {
        res.status(200).send(responseUtil.successFalse(resCode.BAD_REQUEST, resMessage.ALREADY_CUSTOM_PLAYLIST));
    } else {
        await playlist.create({
            ...
            if(err) {
                ...
                res.status(200).send(responseUtil.successTrue(resCode.OK, resMessage.CUSTOM_CREATE_SUCCESS, addCustomResult[0]));
            }
        })
    }
});
